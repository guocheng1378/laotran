...
> **可选增强：Google Cloud Speech-to-Text**
> 项目内内置了一个可选的 Google Cloud STT 识别封装（`GoogleStt.kt`，老挝语 `lo-LA`、16 kHz PCM）。如果觉得默认老挝语识别不理想，可以到 [Google Cloud 控制台](https://console.cloud.google.com/speech-to-text) 开通 Speech-to-Text 并创建 **API Key**，填入 **⚙️ 设置里的 Google STT Key** 输入框。这是**可选**的增强/备用路径，不填时语音输入走系统语音框。
...
├── GoogleStt.kt             （可选）Google 语音识别
├── Config.kt                运行时配置（接口地址 / API Key / 模型）
